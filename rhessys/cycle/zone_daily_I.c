				  ...
				  struct 	date 			current_date)
{
	...
	while ( (i < zone[0].num_base_stations) && (flag<3) ){
		...
		/*																*/
		/*		we do not adjust for slope, cloudiness or lai as yet	*/
		/*--------------------------------------------------------------*/
		temp = zone[0].base_stations[i][0].daily_clim[0].rain[day];
		/*--------------------------------------------------------------*/
		/* 	allow for stochastic noise in precip scaling 		*/
		/*--------------------------------------------------------------*/
		...
		} /* end stochastic noise addition */


		if ( (temp != -999.0) || (temp != -9999.0) ){  // T.N.
	...
	/* Re-use z_delta */
    /*160419LML seems there is an assumption that each zone has one station.*/
    if ((command_line[0].gridded_netcdf_flag == 1) && (world[0].base_station_ncheader[0].elevflag == 0)) {
		z_delta = 0.0;
	}
	...
	/*	We need this since the LAI compensation for temp assumes	*/
	...
	/*  air temperatures and precip as given in Appendix D, EQ. 3	*/
	...
	if ( zone[0].base_stations[0][0].daily_clim[0].Delta_T != NULL ){
		...
		/*		there is a substantial amount of rain (>1inch)			*/
