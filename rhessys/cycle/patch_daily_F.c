						  ...
						  struct	date 			current_date)
{
	...
	*/
	...
	if (zone[0].hourly_rain_flag!=1) {
		...
		//patch[0].recharge = infiltration; // T.N. Apr 2016 recharge calculated below
  ...
  patch[0].target_status = 1;
	...
	/*--------------------------------------------------------------*/
	if (patch[0].sat_deficit < ZERO) { // T.N. Apr 2016 water table is at or above the soil surface, gw discharge & no recharge
		patch[0].S = 1.0;
        ...
        // T.N. Apr 2016: water table is in unsaturated zone, recharge = unsat_drainage
		/*-------------------------------------------------------*/
		...
		patch[0].recharge += unsat_drainage; // T.N Apr. 2016
	}
	else  {
        // T.N Apr. 2016 water table is in the rootzone, recharge = rz_drainage
